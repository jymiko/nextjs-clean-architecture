...
function StatsCard({ title, value, icon, valueColor = "default" }: StatsCardProps) {
  return (
    <div className="bg-white border border-[#e9f5fe] rounded-xl p-4 lg:p-5 flex items-center justify-between flex-1 min-w-[200px]">
      ...
      <div className="size-[50px] lg:size-[54px] shrink-0">{icon}</div>
    </div>
  );
}

interface DistributedDocumentStatsProps {
  totalDocuments: number;
  approvedDocuments: number;
  pendingDocuments: number;
}

export function DistributedDocumentStats({
  totalDocuments,
  approvedDocuments,
  pendingDocuments,
}: DistributedDocumentStatsProps) {
  return (
...
function TotalDocumentsIcon() {
  return (
    <svg width="54" height="54" viewBox="0 0 54 54" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <g clipPath="url(#clip0_total_distributed)">
      ...
      <defs>
        <clipPath id="clip0_total_distributed">
