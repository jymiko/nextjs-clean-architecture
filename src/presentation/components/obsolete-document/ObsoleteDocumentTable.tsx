...
interface ObsoleteDocumentTableProps {
  documents: ObsoleteDocument[];
  onViewDocument?: (document: ObsoleteDocument) => void;
  isLoading?: boolean;
}

export function ObsoleteDocumentTable({
  documents,
  onViewDocument,
  isLoading,
}: ObsoleteDocumentTableProps) {
  if (isLoading) {
    return (
        ...
        <Table>
          ...
          <TableBody>
            {documents.map((doc) => (
              <TableRow
                key={doc.id}
                className="border-[#e1e2e3] h-[68px] cursor-pointer hover:bg-gray-50"
              ...
              >
                ...
                <TableCell className="text-[#384654] text-sm max-w-[250px]">
                  {doc.title}
                </TableCell>
                <TableCell className="text-[#384654] text-sm">
                  {doc.type}
                </TableCell>
                <TableCell className="text-[#384654] text-sm max-w-[180px]">
                  {doc.department}
                </TableCell>
              ...
              <TableRow key={doc.id} className="border-[#e1e2e3]">
                ...
                <TableCell className="text-[#384654] text-xs">
                  <div className="space-y-1">
                    <div className="text-[#738193] text-[10px]">Created By</div>
                    <div>{doc.createdBy}</div>
                    <div className="text-[#738193] text-[10px]">Effective</div>
                    <div>{doc.effectiveDate}</div>
                    <div className="text-[#738193] text-[10px]">Obsolete</div>
                    <div>{doc.obsoleteDate}</div>
                  </div>
                </TableCell>
                <TableCell className="text-[#384654] text-xs max-w-[120px]">
                  {doc.remarks}
                </TableCell>
                <TableCell>
                  <Button
          ...
          {documents.map((doc) => (
            <div key={doc.id} className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  <span className="text-[#738193] text-xs block">Effective Date</span>
                  <p className="text-[#384654] text-sm">{doc.effectiveDate}</p>
                </div>
                <div>
                  <span className="text-[#738193] text-xs block">Obsolete Date</span>
                  <p className="text-[#384654] text-sm">{doc.obsoleteDate}</p>
                </div>
                <div>
                  <span className="text-[#738193] text-xs block">Remarks</span>
                  <p className="text-[#384654] text-sm line-clamp-2">{doc.remarks}</p>
                </div>
              </div>
